...
public class TestMutableMetrics {
  ...
  @Test public void testMutableRates() {
    ...
    assertCounter("FooNumOps", 0L, (MetricsSource) rb);
    assertGauge("FooAvgTime", 0.0, (MetricsSource) rb);
    assertCounter("BarNumOps", 0L, (MetricsSource) rb);
    assertGauge("BarAvgTime", 0.0, (MetricsSource) rb);
  }
  ...
  @Test public void testMutableRatesWithAggregationManyThreads()
    ...
    final org.apache.hadoop.metrics2.lib.MutableRatesWithAggregation rates = new org.apache.hadoop.metrics2.lib.MutableRatesWithAggregation();
  ...
  public void testMutableStatWithBulkAdd() {
    ...
    for (int bulkSize : new int[] {1, 10, 100, 1000}) {
      ...
      assertCounter("TestNumOps", 2000L, (MetricsSource) rb);
      assertGauge("TestAvgVal", stats.mean(), (MetricsSource) rb);
      assertGauge("TestStdevVal", stats.sampleStandardDeviation(), (MetricsSource) rb);
    }
  }
  ...
  /**
   * Ensure that quantile estimates from {@link org.apache.hadoop.metrics2.lib.MutableQuantiles} are within
  ...
  private static void assertCounter(String name, int expected, MetricsRecordBuilder rb) {
    assertThat(getIntCounter(name, rb)).as(name)
        .isEqualTo(expected);
  }

  private static int getIntCounter(String name, MetricsRecordBuilder rb) {
    ArgumentCaptor<Integer> captor = ArgumentCaptor.forClass(
        Integer.class);
    verify(rb, atLeast(0)).addCounter(eqName(Interns.info(name, "")), captor.capture());
